(ns uipath.main
  "Kotodama WASM entrypoint for the uipath clean-room actor (L4) — Clojure port.
  ...
  facts (`uipath.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "uipath")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"uipath.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "uipath-compat" :tier tier :entities entities} 200])
...
(defn start! [] :uipath-compat/ready)
